...
def dispatch_to_nn(file, df, args):
    ...
    if 'featureset_permutation_range' not in args:
        ...
        args['featureset_permutation_range'] = int(args['featureset_permutation_range'][0])
    ...
    features_cli_dispatch_iterations = [
        "--features " + ' '.join(feature_iteration) for feature_iteration in feature_iterations]

    # setup the .csv log file
    datetime_file_name = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    cli_args_filename = [str(k) + str(v) for k, v in args.items()
                        if 'features' not in k and 'featureset_permutation_range' not in k]
    csv_log_filename = '_'.join(cli_args_filename) + '_' + datetime_file_name + '.csv'

    range_dir = "range_" + str(len(f_col)) + '_' + str(args['featureset_permutation_range'])
    layers_permutations_dir = "ylayerspermutations" if args['layers_permutations'] else "nlayerspermutations"
    results_log_path = pathlib.Path('results', args['nn'], range_dir, layers_permutations_dir)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Neural Network launcher script. Dispatches different subsets/permutations of featuresets, layers, and other tweaks to the selected neural network")
    parser.add_argument('--nn', choices=['bpnn', 'rnn'], default='bpnn')
    parser.add_argument('--dbm', action='store_true', help="set this flag to include dbm entries into the featureset")
    parser.add_argument('--epochs', type=int, default=10)
    parser.add_argument('--batch_size', type=int, default=20)
    parser.add_argument('--early_stopping', nargs=2, default=argparse.SUPPRESS, metavar=('MONITOR', 'PATIENCE'))
    parser.add_argument('--features', nargs='+', type=str, default=argparse.SUPPRESS)
    parser.add_argument('--featureset_permutation_range', nargs=1, type=int, default=argparse.SUPPRESS, help="range of the feature-subset(s). range(len(FEATURES) - FEATURESET_PERMUTATION_RANGE")
    ...
    for i in range(len(dataframes)):
        dispatch_to_nn(files[i], dataframes[i], args.copy())
